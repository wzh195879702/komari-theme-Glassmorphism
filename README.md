...
## 📡 v3.3.7 首页 Ping 摘要兼容修复

- 修复首页节点卡片在历史 Ping 数据为空时始终显示灰色占位条和 `-` 的问题
- 首页延迟和丢包按任务逐行展示，联通、电信、移动不再被合并成单个平均值
- 保留后端实时状态中的 Ping 任务汇总，在历史数据不可用时显示各任务最新延迟和丢包
- 历史 Metric Store / Ping records 恢复后仍优先使用历史统计，不改变既有曲线口径
...
<details open>
<summary><strong>v3.3.7 · 首页 Ping 摘要兼容修复</strong></summary>

- 修复历史 Ping 数据为空时首页只显示灰色占位条和 `-` 的问题
- 首页保留每个 Ping 任务的名称、历史格和当前值，三网不再聚合成单行
- 使用节点实时 Ping 任务汇总作为各任务历史数据不可用时的显示回退
- 历史 Metric Store / Ping records 恢复后仍优先使用历史统计
